...
use sodium_sys::FunctionPointers;
...
struct RandomBytesImpl {
    function_pointers: FunctionPointers,
    name: CString,
    seed: [u32; 4],
}
...
impl Default for RandomBytesImpl {
    fn default() -> RandomBytesImpl {
        ...
        let function_pointers = FunctionPointers {
            implementation_name: ffi::implementation_name,
            random: ffi::random,
            stir: None,
            uniform: None,
            buf: ffi::buf,
...
/// Returns the libsodium version.
#[allow(unsafe_code)]
pub fn libsodium_version() -> String {
    let version = unsafe { CStr::from_ptr(sodium_sys::sodium_version_string()).to_bytes() };
    unwrap!(str::from_utf8(version)).to_owned()
}
...
pub fn init_with_rng<T: Rng>(rng: &mut T) -> Result<(), i32> {
    ...
    let seed = [rng.gen(), rng.gen(), rng.gen(), rng.gen()];
    {
        ...
        sodium_result = unsafe {
            sodium_sys::randombytes_set_implementation(&mut random_bytes.function_pointers)
        };
    }
    match sodium_result {
        0 => sodium_result = unsafe { sodium_sys::sodium_init() },
        _ => (),
    };
...
mod tests {
    ...
    #[test]
    fn seeded() {
        println!("libsodium version: {}", libsodium_version());
